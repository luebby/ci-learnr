...
## Outlook: Testable implications

The assumed causal structure of the data generating process implies some (condtional) independencies:
...
As an example we test 

$$H_0: \texttt{height}{\perp\!\!\!\perp}\texttt{smoke}|\texttt{age}, \texttt{sex}$$


Again we assume a linear model and do a permutation test:

```{r permuteindep, warning=FALSE , exercise = TRUE}
lm(height ~ smoke + sex + age, data = FEV)

set.seed(1896)

Nulldist <- do(1000) * lm(height ~ shuffle(smoke) + sex + age, data = FEV)

gf_histogram( ~ smokeyes, data = Nulldist, center = 0, bins = 21) %>%
  gf_vline(xintercept =  -0.6404)
```

```{r pvalueci, echo=FALSE}
question("Is the estimate ($\\hat{\\beta}_{\\text{smokeyes}}=-0.6404$) (very) incompatible to the model of no conditional association?",
  answer("Yes.", message = "The estimated coefficient is not extreme if there would be no conditional association."),
  answer("No.", correct = TRUE, message = "The estimated coefficient is not extreme if there would be no conditional association.")
)
```
...
[DAGitty](http://dagitty.net/) returns Minimal sufficient adjustment sets for a given graph as well as an R code for testable implications via `ggm::pcor.test()`:
...
So the $H_0: \texttt{fev}{\perp\!\!\!\perp}\texttt{age}|\texttt{height}, \texttt{sex}, \texttt{smoke}$ would be rejected and the model should be refined!
