...
[DAGitty](http://dagitty.net/) returns minimal sufficient adjustment sets for a given graph as well as a R code for testable implications via `ggm::pcor.test()`:
...
So $H_0: \texttt{fev}{\perp\!\!\!\perp}\texttt{age}|\texttt{height}, \texttt{sex}, \texttt{smoke}$ would be rejected and the model should be refined!
